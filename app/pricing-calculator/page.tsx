...
export default function PricingCalculator() {
  ...
  const calculateServicePricing = () => {
    ...
    // Calculate required rate based on desired salary
    const requiredGrossIncome = salary / (1 - tax / 100)
    const requiredRevenue = requiredGrossIncome + annualExpenses
    const requiredHourlyRate = requiredRevenue / totalBillableHours

    // Calculate break-even rate
    const breakEvenRate = annualExpenses / totalBillableHours
    ...
    setResult({
      ...
      effectiveHourlyRate: netIncome / totalBillableHours,
      industryData,
      isCompetitive,
      scenarios,
      profitMarginActual: (grossProfit / annualRevenue) * 100 || 0,
    })
  }
    ...
    const strategies = [
      ...
      { name: "Penetration", price: sellingPrice * 0.8, description: "20% below standard for market entry" },
        ...
        .map((strategy) => ({
          ...
          margin: ((strategy.price - cost) / strategy.price) * 100,
        }))
                                  ...
                                  {result.scenarios.map((scenario: any, index: number) => (
                                      <div key={index} className="flex items-center justify-between p-2 bg-white rounded">
                                        ...
                                        <div className="text-right">
                                          <div className="font-semibold">${scenario.annualRevenue.toLocaleString()}</div>
                                          <div
                                              className={`text-sm ${scenario.netIncome >= 0 ? "text-green-600" : "text-red-600"}`}
                                          >
                                            Net: ${scenario.netIncome.toLocaleString()}
                                          </div>
                                        </div>
                                      </div>
                                  ))}
