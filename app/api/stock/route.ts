...
export async function GET(req: NextRequest) {
    ...
    if (!apiKey) {
        return NextResponse.json({ error: "Polygon API key is not configured on the server." }, { status: 500 });
    }
    ...
    try {
        ...
        // --- Error Checks (Accepts "DELAYED" status) ---
        ...
        const combinedData = {
            quote: {
                "01. symbol": symbol,
                "05. price": prevDay.c,
                "09. change": (prevDay.c - prevDay.o).toFixed(2),
            ...
            overview: {
                ...
                // Polygon free tier does not provide a logo, so we'll construct a URL.
                // Note: This is a common pattern but might not work for all stocks.
                Logo: `https://logo.clearbit.com/${profileData.results.homepage_url?.split('//')[1]}`,
            },
            timeSeries: timeSeriesData.results.reduce((acc: any, bar: any) => {
                const date = format(new Date(bar.t), 'yyyy-MM-dd');
                acc[date] = {
                    "1. open": bar.o, "2. high": bar.h, "3. low": bar.l, "4. close": bar.c, "5. volume": bar.v,
                };
                return acc;
            }, {}),
        };
